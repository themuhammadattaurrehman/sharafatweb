...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $existingEmailQuery = "SELECT * FROM admin WHERE email = :email
                        UNION
                        SELECT * FROM cashier WHERE email = :email
                        UNION
                        SELECT * FROM manager WHERE email = :email
                        UNION
                        SELECT * FROM salesman WHERE email = :email";
    $existingEmailStmt = $conn->prepare($existingEmailQuery);
    $existingEmailStmt->bindParam(':email', $email, PDO::PARAM_STR);
    $existingEmailStmt->execute();
    $existingUser = $existingEmailStmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($existingUser) {
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['success_message'] = "You are successfully signed up.";
